...
int mmc_switch_part(int dev_num, unsigned int part_num)
{
    ...
    debug("%s : EXT_CSD_CMD_SET_NORMAL = 0x%x, EXT_CSD_PART_CONF = 0x%x, para = 0x%x\n", 
                __func__,
                EXT_CSD_CMD_SET_NORMAL,
                EXT_CSD_PART_CONF,
                (0x48 & ~PART_ACCESS_MASK) | (part_num & PART_ACCESS_MASK)
                );
    ret = mmc_switch(mmc, 
        EXT_CSD_CMD_SET_NORMAL, 
        EXT_CSD_PART_CONF,
...
#if 0
    err = mmc_switch_part(0, 1);
	if (err)
		puts("%s :MMC partition switch failed\n", __func__);
    else
        debug("%s :MMC partition switch success\n", __func__);
#endif
    /*
